...
# 下载全市乡村游客源排名里面的数据
class QuanShiXiangCunYouKeYuanPaiMing(Base.Downloader):
    ...
    def fetch_data(self, **kwargs):
        ...
        if self.include_jiangsu:
            resp = requests.post(url=self.host + "/kyfx/info/XiangcunyouKeYuanPaiMing.jsp.jsp",
                                 params={"var": war, "data": time_interval, "outNj": out_nj},
                                 headers={"cookie": self.cookie})
        else:
            resp = requests.post(url=self.host + "/kyfx/info/XiangcunyouKeYuanPaiMing.jsp",
                                 params={"var": war, "data": time_interval},
                                 headers={"cookie": self.cookie})

        resp = requests.post(url=self.host + "/kyfx/info/XiangcunyouKeYuanPaiMing1.jsp",
                             params={"war": war, "outNj": out_nj}, headers={"cookie": self.cookie})
        resp.raise_for_status()

        resp = requests.post(url=self.host + "/kyfx/info/xiazai.jsp",
                             params={"var": "jqkeyuanxcykeyuan"}, headers={"cookie": self.cookie})
        resp.raise_for_status()
    ...
    def pipeline(self, continuous=False, set_cookie=True, **kwargs):
        if ("year_head" not in kwargs) or ("year_tail" not in kwargs) or ("month_head" not in kwargs) \
            ...
            print("Arguments must contain: year_head, year_tail, month_head, month_tail, day_head,"
                  "day_tail, include_jiangsu, include_nanjing")
            return
        if set_cookie:
            self.set_cookie()
        else:
...
if __name__ == "__main__":
    downloader = QuanShiXiangCunYouKeYuanPaiMing()
